...
class MainMenu(BetterCMD):
	...
	def set_id(self, ID):
		...
		self.prompt = (
				f"{paint(f'(').cyan_DIM}{paint('Pawncat').magenta}{paint(f')').cyan_DIM}"
	...
	def do_kill(self, ID):
		...
		if options.single_session and len(core.sessions) == 1:
			core.stop()
			logger.info("Pawncat exited due to Single Session mode")
			return True
	...
	def do_spawn(self, line):
		...
		Examples:
			...
			spawn 5555		Spawn a reverse shell on 5555 port. This can be used to get shell
						on another tab. On the other tab run: ./Pawncat.py 5555
	...
	def do_exit(self, line):
		...
		Exit Pawncat
		"""
		if ask(f"Exit Pawncat?{self.active_sessions} (y/N): ").lower() == 'y':
			super().do_exit(line)
			core.stop()
...
def custom_excepthook(*args):
	...
	print('─' * 80, f"\n{paint('Pawncat version:').red} {paint(__version__).green}")
...
def load_rc():
	RC = Path(options.basedir / "pawncatrc")
	try:
...
def main():
	...
	parser = ArgumentParser(description="Pawncat Shell Handler", add_help=False)
	...
	debug.add_argument("-v", "--version", help="Show Pawncat version", action="store_true")
...
if not sys.version_info >= (3, 6):
	print("(!) Pawncat requires Python version 3.6 or higher (!)")
	sys.exit(1)
